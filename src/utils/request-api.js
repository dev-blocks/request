...
function covertEndpointDefinitionToId(def, path) {
	return def
		.substr(path.length)
		.replace(/\(\?P<([a-zA-Z_-]+)>(.[^)]+)\)/g, (match, attr, group) => {
			let attrString = `<${attr}>`;
...
export const requestEndpoints = async ({
	path = '/wp/v2',
	filterMethods = [],
}) => {
	return await apiFetch({
		...
		if (routes) {
			return filter(
				values(
					mapValues(routes, (item, endpoint) => {
						return {
							...item,
...
export const requestNamespaces = requestEndpoints({
	path: '/',
})
	.then((namespaces) => {
		return map(namespaces, (namespace) => ({
			label: namespace,
			value: namespace,
		}));
	})
